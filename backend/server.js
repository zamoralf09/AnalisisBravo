...
async function crearTablasSistema() {
    ...
    const tablasBase = [
        ...
        // Nuevas tablas de cuenta corriente
        ...
        `CREATE TABLE TIPO_MOVIMIENTO_CXC (
            ...
            Nombre varchar(100) not null,
            OperacionCuentaCorriente int not null,
            FechaCreacion datetime not null,
            UsuarioCreacion varchar(100) not null,
            FechaModificacion datetime,
            UsuarioModificacion varchar(100),
    ...
    const tablasDependientes = [
        ...
        // Tabla PERSONA (requiere GENERO y ESTADO_CIVIL)
        ...
        // Tabla DOCUMENTO_PERSONA (requiere PERSONA y TIPO_DOCUMENTO)
        `CREATE TABLE DOCUMENTO_PERSONA (
            IdDocumentoPersona int not null auto_increment,
            ...
            FechaModificacion datetime,
            UsuarioModificacion varchar(100),
            primary key (IdDocumentoPersona),
...
async function insertarDatosIniciales() {
    try {
        ...
        // STATUS_CUENTA
        const [statusCuentaRows] = await db.execute('SELECT COUNT(*) as total FROM STATUS_CUENTA');
        if (statusCuentaRows[0].total === 0) {
            await db.execute("INSERT INTO STATUS_CUENTA (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Cuenta Activa', NOW(), 'system')");
            await db.execute("INSERT INTO STATUS_CUENTA (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Cuenta Cancelada', NOW(), 'system')");
            await db.execute("INSERT INTO STATUS_CUENTA (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Cuenta en cobro judicial', NOW(), 'system')");
        }

        // ESTADO_CIVIL
        const [estadoCivilRows] = await db.execute('SELECT COUNT(*) as total FROM ESTADO_CIVIL');
        if (estadoCivilRows[0].total === 0) {
            await db.execute("INSERT INTO ESTADO_CIVIL (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Soltero', NOW(), 'system')");
            await db.execute("INSERT INTO ESTADO_CIVIL (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Casado', NOW(), 'system')");
            await db.execute("INSERT INTO ESTADO_CIVIL (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Divorciado', NOW(), 'system')");
            await db.execute("INSERT INTO ESTADO_CIVIL (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Viudo', NOW(), 'system')");
            await db.execute("INSERT INTO ESTADO_CIVIL (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Otro', NOW(), 'system')");
        }

        // TIPO_DOCUMENTO
        const [tipoDocumentoRows] = await db.execute('SELECT COUNT(*) as total FROM TIPO_DOCUMENTO');
        if (tipoDocumentoRows[0].total === 0) {
            await db.execute("INSERT INTO TIPO_DOCUMENTO (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('DPI', NOW(), 'system')");
            await db.execute("INSERT INTO TIPO_DOCUMENTO (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Pasaporte', NOW(), 'system')");
            await db.execute("INSERT INTO TIPO_DOCUMENTO (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Licencia de conducir', NOW(), 'system')");
        }

        // TIPO_MOVIMIENTO_CXC
        const [tipoMovimientoRows] = await db.execute('SELECT COUNT(*) as total FROM TIPO_MOVIMIENTO_CXC');
        if (tipoMovimientoRows[0].total === 0) {
            await db.execute("INSERT INTO TIPO_MOVIMIENTO_CXC (Nombre, OperacionCuentaCorriente, FechaCreacion, UsuarioCreacion) VALUES ('Cargo', 1, NOW(), 'system')");
            await db.execute("INSERT INTO TIPO_MOVIMIENTO_CXC (Nombre, OperacionCuentaCorriente, FechaCreacion, UsuarioCreacion) VALUES ('Abono', -1, NOW(), 'system')");
        }

        // TIPO_SALDO_CUENTA
        const [tipoSaldoRows] = await db.execute('SELECT COUNT(*) as total FROM TIPO_SALDO_CUENTA');
        if (tipoSaldoRows[0].total === 0) {
            await db.execute("INSERT INTO TIPO_SALDO_CUENTA (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Saldo Actual', NOW(), 'system')");

            await db.execute("INSERT INTO TIPO_SALDO_CUENTA (Nombre, FechaCreacion, UsuarioCreacion) VALUES ('Saldo Vencido', NOW(), 'system')");
        }
...
app.get('/api/usuarios', async (req, res) => {
    try {
        const [rows] = await db.execute(`
            SELECT IdUsuario, Nombre, Apellido, FechaNacimiento, IdStatusUsuario, IdGenero, UltimaFechaIngreso, IntentosDeAcceso, SesionActual, UltimaFechaCambioPassword, CorreoElectronico, RequiereCambiarPassword, TelefonoMovil, IdSucursal, Pregunta, Respuesta, IdRole, FechaCreacion, UsuarioCreacion, FechaModificacion, UsuarioModificacion
            FROM USUARIO`);
        res.json(rows);
    } catch (error) {
...
app.get('/api/personas', async (req, res) => {
    try {
        const [rows] = await db.execute(`
            SELECT p.*, g.Nombre AS GeneroNombre, e.Nombre AS EstadoCivilNombre
            FROM PERSONA p
            JOIN GENERO g ON p.IdGenero = g.IdGenero
            JOIN ESTADO_CIVIL e ON p.IdEstadoCivil = e.IdEstadoCivil
...
app.get('/api/personas/:id', async (req, res) => {
    try {
        ...
        const [docs] = await db.execute('SELECT * FROM DOCUMENTO_PERSONA WHERE IdPersona = ?', [id]);
        rows[0].documentos = docs;
        res.json(rows[0]);
    } catch (error) {
...
app.post('/api/personas', async (req, res) => {
    try {
        const { Nombre, Apellido, FechaNacimiento, IdGenero, Direccion, Telefono, CorreoElectronico, IdEstadoCivil, documentos } = req.body;
        if (!Nombre || !Apellido || !FechaNacimiento || !IdGenero || !Direccion || !Telefono || !IdEstadoCivil) {
        ...
        const [result] = await db.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)`,
            [Nombre, Apellido, FechaNacimiento, IdGenero, Direccion, Telefono, CorreoElectronico, IdEstadoCivil, req.user?.email || 'system']
        );
        const idPersona = result.insertId;
        // Insertar documentos
        if (Array.isArray(documentos)) {
            for (const doc of documentos) {
                if (doc.IdTipoDocumento && doc.NoDocumento) {
                    await db.execute(
                        'INSERT INTO DOCUMENTO_PERSONA (IdPersona, IdTipoDocumento, NoDocumento, FechaCreacion, UsuarioCreacion) VALUES (?, ?, ?, NOW(), ?)',
                        [idPersona, doc.IdTipoDocumento, doc.NoDocumento, req.user?.email || 'system']
                    );
                }
            }
        }
        res.json({ success: true, IdPersona: idPersona });
    } catch (error) {
...
app.put('/api/personas/:id', async (req, res) => {
    try {
        ...
        const [result] = await db.execute(
            `UPDATE PERSONA SET Nombre=?, Apellido=?, FechaNacimiento=?, IdGenero=?, Direccion=?, Telefono=?, CorreoElectronico=?, IdEstadoCivil=?, FechaModificacion=NOW(), UsuarioModificacion=? WHERE IdPersona=?`,
            [Nombre, Apellido, FechaNacimiento, IdGenero, Direccion, Telefono, CorreoElectronico, IdEstadoCivil, req.user?.email || 'system', id]
        );
        // Actualizar documentos: eliminar y volver a insertar
        await db.execute('DELETE FROM DOCUMENTO_PERSONA WHERE IdPersona = ?', [id]);
        if (Array.isArray(documentos)) {
            for (const doc of documentos) {
                if (doc.IdTipoDocumento && doc.NoDocumento) {
                    await db.execute(
                        'INSERT INTO DOCUMENTO_PERSONA (IdPersona, IdTipoDocumento, NoDocumento, FechaCreacion, UsuarioCreacion) VALUES (?, ?, ?, NOW(), ?)',
                        [id, doc.IdTipoDocumento, doc.NoDocumento, req.user?.email || 'system']
                    );
                }
            }
        }
...
app.delete('/api/personas/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await db.execute('DELETE FROM DOCUMENTO_PERSONA WHERE IdPersona = ?', [id]);
        const [result] = await db.execute('DELETE FROM PERSONA WHERE IdPersona = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Persona no encontrada' });
        }
        res.json({ success: true });
...
// ===================== CRUD STATUS_CUENTA =====================
app.get('/api/status-cuentas', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM STATUS_CUENTA ORDER BY Nombre');
        res.json(rows);
    } catch (error) {
        console.error('Error obteniendo status de cuentas:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.get('/api/status-cuentas/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await db.execute('SELECT * FROM STATUS_CUENTA WHERE IdStatusCuenta = ?', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Status de cuenta no encontrado' });
        }
        res.json(rows[0]);
    } catch (error) {
        console.error('Error obteniendo status de cuenta por ID:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.post('/api/status-cuentas', async (req, res) => {
    try {
        const { Nombre } = req.body;
        if (!Nombre) {
            return res.status(400).json({ error: 'El nombre del status de cuenta es requerido' });
        }
        const [result] = await db.execute('INSERT INTO STATUS_CUENTA (Nombre, FechaCreacion, UsuarioCreacion) VALUES (?, NOW(), ?)', [Nombre, req.user?.email || 'system']);
        res.json({ success: true, IdStatusCuenta: result.insertId });
    } catch (error) {
        console.error('Error creando status de cuenta:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.put('/api/status-cuentas/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { Nombre } = req.body;
        if (!Nombre) {
        ...
        const [result] = await db.execute('UPDATE STATUS_CUENTA SET Nombre = ?, FechaModificacion = NOW(), UsuarioModificacion = ? WHERE IdStatusCuenta = ?', [Nombre, req.user?.email || 'system', id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Status de cuenta no encontrado' });
        }
        res.json({ success: true });
    } catch (error) {
        console.error('Error actualizando status de cuenta:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.delete('/api/status-cuentas/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await db.execute('DELETE FROM STATUS_CUENTA WHERE IdStatusCuenta = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Status de cuenta no encontrado' });
        }
        res.json({ success: true });
    } catch (error) {
        console.error('Error eliminando status de cuenta:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

// ===================== CRUD ESTADO_CIVIL =====================
app.get('/api/estado-civil', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM ESTADO_CIVIL ORDER BY Nombre');
        res.json(rows);
    } catch (error) {
...
app.get('/api/estado-civil/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await db.execute('SELECT * FROM ESTADO_CIVIL WHERE IdEstadoCivil = ?', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Estado civil no encontrado' });
        }
        res.json(rows[0]);
    } catch (error) {
        console.error('Error obteniendo estado civil por ID:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.post('/api/estado-civil', async (req, res) => {
    try {
        const { Nombre } = req.body;
        if (!Nombre) {
            return res.status(400).json({ error: 'El nombre del estado civil es requerido' });
        }
        const [result] = await db.execute('INSERT INTO ESTADO_CIVIL (Nombre, FechaCreacion, UsuarioCreacion) VALUES (?, NOW(), ?)', [Nombre, req.user?.email || 'system']);
        res.json({ success: true, IdEstadoCivil: result.insertId });
    } catch (error) {
...
app.put('/api/estado-civil/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { Nombre } = req.body;
        if (!Nombre) {
        ...
        const [result] = await db.execute('UPDATE ESTADO_CIVIL SET Nombre = ?, FechaModificacion = NOW(), UsuarioModificacion = ? WHERE IdEstadoCivil = ?', [Nombre, req.user?.email || 'system', id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Estado civil no encontrado' });
        }
        res.json({ success: true });
...
app.delete('/api/estado-civil/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await db.execute('DELETE FROM ESTADO_CIVIL WHERE IdEstadoCivil = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Estado civil no encontrado' });
        }
        res.json({ success: true });
...
app.get('/api/tipos-documento', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM TIPO_DOCUMENTO ORDER BY Nombre');
        res.json(rows);
    } catch (error) {
...
app.get('/api/tipos-documento/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await db.execute('SELECT * FROM TIPO_DOCUMENTO WHERE IdTipoDocumento = ?', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Tipo de documento no encontrado' });
        }
        res.json(rows[0]);
    } catch (error) {
        console.error('Error obteniendo tipo de documento por ID:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});
...
app.post('/api/tipos-documento', async (req, res) => {
    try {
        ...
        const [result] = await db.execute('INSERT INTO TIPO_DOCUMENTO (Nombre, FechaCreacion, UsuarioCreacion) VALUES (?, NOW(), ?)', [Nombre, req.user?.email || 'system']);
        res.json({ success: true, IdTipoDocumento: result.insertId });
    } catch (error) {
...
app.put('/api/tipos-documento/:id', async (req, res) => {
    try {
        ...
        const [result] = await db.execute('UPDATE TIPO_DOCUMENTO SET Nombre = ?, FechaModificacion = NOW(), UsuarioModificacion = ? WHERE IdTipoDocumento = ?', [Nombre, req.user?.email || 'system', id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tipo de documento no encontrado' });
        }
        res.json({ success: true });
...
app.delete('/api/tipos-documento/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await db.execute('DELETE FROM TIPO_DOCUMENTO WHERE IdTipoDocumento = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tipo de documento no encontrado' });
        }
        res.json({ success: true });
...
// ===================== CRUD TIPO_MOVIMIENTO_CXC =====================
app.get('/api/tipos-movimiento-cxc', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM TIPO_MOVIMIENTO_CXC ORDER BY Nombre');
        res.json(rows);
    } catch (error) {
        console.error('Error obteniendo tipos de movimiento CxC:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.get('/api/tipos-movimiento-cxc/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await db.execute('SELECT * FROM TIPO_MOVIMIENTO_CXC WHERE IdTipoMovimientoCXC = ?', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Tipo de movimiento CxC no encontrado' });
        }
        res.json(rows[0]);
    } catch (error) {
        console.error('Error obteniendo tipo de movimiento CxC por ID:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.post('/api/tipos-movimiento-cxc', async (req, res) => {
    try {
        const { Nombre, OperacionCuentaCorriente } = req.body;
        if (!Nombre || !OperacionCuentaCorriente) {
            return res.status(400).json({ error: 'Nombre y OperacionCuentaCorriente son requeridos' });
        }
        const [result] = await db.execute('INSERT INTO TIPO_MOVIMIENTO_CXC (Nombre, OperacionCuentaCorriente, FechaCreacion, UsuarioCreacion) VALUES (?, ?, NOW(), ?)', [Nombre, OperacionCuentaCorriente, req.user?.email || 'system']);
        res.json({ success: true, IdTipoMovimientoCXC: result.insertId });
    } catch (error) {
        console.error('Error creando tipo de movimiento CxC:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.put('/api/tipos-movimiento-cxc/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { Nombre, OperacionCuentaCorriente } = req.body;
        if (!Nombre || !OperacionCuentaCorriente) {
        ...
        const [result] = await db.execute('UPDATE TIPO_MOVIMIENTO_CXC SET Nombre = ?, OperacionCuentaCorriente = ?, FechaModificacion = NOW(), UsuarioModificacion = ? WHERE IdTipoMovimientoCXC = ?', [Nombre, OperacionCuentaCorriente, req.user?.email || 'system', id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tipo de movimiento CxC no encontrado' });
        }
        res.json({ success: true });
    } catch (error) {
        console.error('Error actualizando tipo de movimiento CxC:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.delete('/api/tipos-movimiento-cxc/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await db.execute('DELETE FROM TIPO_MOVIMIENTO_CXC WHERE IdTipoMovimientoCXC = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tipo de movimiento CxC no encontrado' });
        }
        res.json({ success: true });
    } catch (error) {
        console.error('Error eliminando tipo de movimiento CxC:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

// ===================== CRUD TIPO_SALDO_CUENTA =====================
app.get('/api/tipos-saldo-cuenta', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM TIPO_SALDO_CUENTA ORDER BY Nombre');
        res.json(rows);
    } catch (error) {
        console.error('Error obteniendo tipos de saldo de cuenta:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.get('/api/tipos-saldo-cuenta/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await db.execute('SELECT * FROM TIPO_SALDO_CUENTA WHERE IdTipoSaldoCuenta = ?', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Tipo de saldo de cuenta no encontrado' });
        }
        res.json(rows[0]);
    } catch (error) {
        console.error('Error obteniendo tipo de saldo de cuenta por ID:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.post('/api/tipos-saldo-cuenta', async (req, res) => {
    try {
        const { Nombre } = req.body;
        if (!Nombre) {
            return res.status(400).json({ error: 'El nombre del tipo de saldo de cuenta es requerido' });
        }
        const [result] = await db.execute('INSERT INTO TIPO_SALDO_CUENTA (Nombre, FechaCreacion, UsuarioCreacion) VALUES (?, NOW(), ?)', [Nombre, req.user?.email || 'system']);
        res.json({ success: true, IdTipoSaldoCuenta: result.insertId });
    } catch (error) {
        console.error('Error creando tipo de saldo de cuenta:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.put('/api/tipos-saldo-cuenta/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { Nombre } = req.body;
        if (!Nombre) {
        ...
        const [result] = await db.execute('UPDATE TIPO_SALDO_CUENTA SET Nombre = ?, FechaModificacion = NOW(), UsuarioModificacion = ? WHERE IdTipoSaldoCuenta = ?', [Nombre, req.user?.email || 'system', id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tipo de saldo de cuenta no encontrado' });
        }
        res.json({ success: true });
    } catch (error) {
        console.error('Error actualizando tipo de saldo de cuenta:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});

app.delete('/api/tipos-saldo-cuenta/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await db.execute('DELETE FROM TIPO_SALDO_CUENTA WHERE IdTipoSaldoCuenta = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Tipo de saldo de cuenta no encontrado' });
        }
        res.json({ success: true });
    } catch (error) {
        console.error('Error eliminando tipo de saldo de cuenta:', error);
        res.status(500).json({ error: 'Error en el servidor' });
    }
});
